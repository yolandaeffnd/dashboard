...
class UserController extends Controller {
        ...
        try {
            if (isset($_POST['pilihanUnit'])) {
                ...
                if (isset($_POST['pilihanUnit'])) {
                    ...
                    $conn->BatchInsert('app_user_data', ['idUser', 'idFak'], $paramsUnit);
                }
                $transaksi->commit();
            } else {
                //Unit
    ...
    public function actionView($id) {
        ...
        $queryUnit->select(['*',
            '(SELECT GROUP_CONCAT(idFak) FROM app_user_data WHERE idUser=:id)AS arrUserData'
        ]);
        $queryUnit->params([':id' => $id]);
        $dataProviderUnitPengelola = new ActiveDataProvider([
            'query' => $queryUnit,
            'pagination' => [
                'pageSize' => 500
            ]
        ...
        return $this->render('view', [
                    ...
                    'dataProviderUnitPengelola' => $dataProviderUnitPengelola
        ]);
    }
